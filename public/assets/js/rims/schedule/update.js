...
$(document).on('change', '#courseSchedRmModal #rm_instructor .rm_instructor1', function (e) {
    rm_instructor_update();
});
$(document).on('change', '#courseSchedRmModal #rm_instructor .rm_instructor_day', function (e) {
    var schedule_id = $('#courseSchedRmModal #schedule select[name="schedule"] option:selected').val();
    if(schedule_id=='new'){
        select_time();
    }else{
