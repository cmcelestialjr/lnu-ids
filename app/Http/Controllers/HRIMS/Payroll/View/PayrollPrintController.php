...
class PayrollPrintController extends Controller
{
    ...
    private function pdf($payroll,$qrcode){
        ...
        $account_title_allowance = AccAccountTitle::
        ...
        if($allowance_list->count()>0){
            ...
            foreach($allowance_list as $allowance){
                ...
                $pdf::SetXY($x+$x_add, $y+$y_add);
                $pdf::Cell($allocated_per_column, 6.6, $allowance->name, 1, 1, 'C', 0, '', 1);
                $allowance_x++;
            }
            $x_add = $x_add+$allocated_per_column;
        }
        ...
        if(count($payroll_list)>0 && $fund_services->count()>0){
                    ...
                    if($fund_service->fund_services_id==$fund_services_id){
                        ...
                        if($allowance_list->count()>0){
                            ...
                            foreach($allowance_list as $allowance){
                                ...
                                $pdf::SetXY($x+$x_add, $y+$y_add);
                                $pdf::Cell($allocated_per_column, 6.6, $allowance_amount, 1, 1, 'R', 0, '', 1);
                                $allowance_x++;
                            }
                            $x_add = $x_add+$allocated_per_column;
                        }
                ...
                if($fund_services->count()>1){
                    ...
                    if($allowance_list->count()>0){
                        ...
                        foreach($allowance_list as $allowance){
                            ...
                            $pdf::SetXY($x+$x_add, $y+$y_add);
                            $pdf::Cell($allocated_per_column, 6.6, $this->check_zero($total_allowance_services[$fund_services_id_main][$allowance->id]), 1, 1, 'R', 0, '', 1);
                            $allowance_x++;
                        }
                        $x_add = $x_add+$allocated_per_column;
                    }
                    ...
                    if($group_services_next!=$fund_service->fund_services->group && $group_services_count>1){
                        ...
                        if($allowance_list->count()>0){
                            ...
                            foreach($allowance_list as $allowance){
                                ...
                                $pdf::SetXY($x+$x_add, $y+$y_add);
                                $pdf::Cell($allocated_per_column, 6.6, $this->check_zero($total_allowance_services_group[$allowance->id]), 1, 1, 'R', 0, '', 1);
                                $allowance_x++;
                            }
                            $x_add = $x_add+$allocated_per_column;
                        }
            ...
            if($allowance_list->count()>0){
                ...
                foreach($allowance_list as $allowance){
                    ...
                    $pdf::SetXY($x+$x_add, $y+$y_add);
                    $pdf::Cell($allocated_per_column, 6.6, $this->check_zero($total_allowance[$allowance->id]), 1, 1, 'R', 0, '', 1);
            ...
            if($fund_services->count()>0){
                ...
                if($account_title_payroll->count()>0){
                    foreach($account_title_payroll as $row){
                        if(($row->column=='w_salary_name' && $row->payroll_type->w_salary=='Yes') ||
                ...
                for ($i = 0; $i < $fund_services->count(); $i++) {
                    ...
                    if($group_services_next!=$fund_service->fund_services->group){
                        ...
                        $pdf::SetXY($x+$x_add-17, $y+$y_add+3);
                        $pdf::Cell(17, '', $this->check_zero($total_salary_services_group1[$fund_service->fund_services_id]), 0, 1, 'R', 0, '', 1);
                        $pdf::SetXY($x+$x_add, $y+$y_add+3);
                        $pdf::Cell(20, '', $this->check_zero($total_netpay_services_group1[$fund_service->fund_services_id]+$total_deduction_services_group1[$fund_service->fund_services_id]), 0, 1, 'R', 0, '', 1);
                ...
                $pdf::Cell(37, '', '', 'TB', 1, 'L', 0, '', 1);
                $pdf::SetXY($x+$x_add-17, $y+$y_add+3);
                $pdf::Cell(17, '', $this->check_zero($total_salary+$total_allowances), 0, 1, 'R', 0, '', 1);
                $pdf::SetXY($x+$x_add, $y+$y_add+3);
            ...
            $x_add = $x_add+15;
            $pdf::SetXY($x+$x_add, $y+$y_add);
            $pdf::Cell(30, '', $this->check_zero($total_earned), 'B', 1, 'C', 0, '', 1);
