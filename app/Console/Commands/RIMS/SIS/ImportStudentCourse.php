...
class ImportStudentCourse extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'app:sis-import-student-course';
    ...
    public function handle()
    {
        ...
        // $studentProgram = StudentsProgram::
        //     where('id','>=',40001)
        //     // ->where('id','<=',40000)
        //     ->get();
        // if($studentProgram->count()>0){
        //     foreach($studentProgram as $row){
        //         $getCurriculum = EducCurriculum::where('year_from','<=',($row->year_from))
        ...
        // $courses = StudentsCourses::with('info')->get();
        // foreach($courses as $course){
        //     StudentsCourses::where('id', $course->id)
        //                 ->update([
        //                     'stud_id' => $course->info->stud_id
        //                 ]);
        // }

        $courses = LudongMark::with('student','school_name','subject','subject_ext')
        ...
        if($courses->count()>0){
            foreach($courses as $course){
                ...
                if($course->school_name){
                    $school_name = $course->school_name->school_name;
                }                
                if($course->school==528){
                    $school_name = 'Leyte Normal University';
                }
                if($course->term==3){
                    $grade_period_id = 4;
                ...
                if($course->catalog_id>0){
                    if($course->subject_ext){
                        $course_desc = $course->subject_ext->desc_title;
                    }
                }else{
                    if($course->subject){
                        $course_desc = $course->subject->description;
                        $units = $course->subject->load_units;
                    }                    
                }
                ...
                $student_program_id = NULL;
                    ...
                    $getGradeBy = LudongMarks::where('stud_id',$course->stud_id)
                        ->where('sy',$course->sy)
                    ...
                    if($getGradeBy){
                        ...
                        $graded_at = date('Y-m-d H:i:s',strtotime($getGradeBy->submitted_on));
                    ...
                    if($course->retake!='' || $course->retake!=NULL){
                        ...
                        if (strpos($course->grade, "INC") !== false) {
                            $inc = "INC";
                        }
                    }else{
                ...
                if($courseCheck==NULL){
                    ...
                    $credit = LudongMarkCredit::where('stud_id',$course->stud_id)
                        ->where('sy',$course->sy)
                        ->where('term',$course->term)
                        ->where('terms',$course->terms)
                        ->where('catalog_no',$course->catalog_no)
