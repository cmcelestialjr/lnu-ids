...
class ImportStudentCourseClass extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'app:sis-import-student-course-class';
    ...
    public function handle()
    {
        ...
        $subjects_offerings = DB::connection($connectionOfferings)
            ...
            ->whereExists(function ($query) {
                $query->select(DB::raw(1))
                    ->from('teachers')
                    ->whereRaw('subjects.sy = teachers.sy')
                    ->whereRaw('subjects.term = teachers.term')
                    ->whereRaw('subjects.terms = teachers.terms')
                    ->whereRaw('subjects.slot_id = teachers.slot_id')
                    ->whereIn('teachers.teacher',[1613, 161017, 161018, 161105, 11516]);
            })
            ->orderBy('sy', 'ASC')
            ->orderBy('term', 'ASC')
            ->orderBy('terms', 'ASC')
        ...
        $class_course = DB::connection($connectionName)
            ...
            ->whereNotExists(function ($query) {
                $query->select(DB::raw(1))
                    ->from('mark')
                    ->whereRaw('classes.stud_id = mark.stud_id')
                    ->whereRaw('classes.catalog_no = mark.catalog_no')
                    ->whereRaw('classes.sy = mark.sy')
                    ->whereRaw('classes.term = mark.term')
                    ->whereRaw('classes.terms = mark.terms');
            })
            ->orderBy('sy', 'ASC')
            ->orderBy('term', 'ASC')
            ->orderBy('terms', 'ASC')
        ...
        $class_course_not_in_subjects_offerings = $class_course->reject(function ($course) use ($subjects_offerings) {
            return $subjects_offerings->where('sy', $course->sy)
        ...
        // $class_course = DB::connection($connectionName)->table('classes')
        //     ->where('sy','2024')
        //     ->where('term','>',0)
        //     ->orderBy('sy','ASC')
        //     ->orderBy('term','ASC')
            ...
            foreach($class_course as $course){
                ...
                if($student){
                    ...
                    if($courseCheck==NULL){
                        ...
                        $ludong_subject = LudongCollegeSubjects::where('catalog_no',$course_code)->first();
                        if($ludong_subject){
                            $units = $ludong_subject->load_units;
                            $course_desc = $ludong_subject->description;
                        }
